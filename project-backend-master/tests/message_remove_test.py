...
##############################################TESTS################################################
###################################################################################################
def test_message_remove_invalid_token(clear__register3users_create4channels):
    user1_send = {'token': encode_jwt(1,0), 'channel_id' : 1, 'message':"hey guys"}
    msg_request = requests.post(f"{config.url}message/send/v1", json=user1_send)
    assert msg_request.status_code == 200
    msg_data = msg_request.json()
    assert msg_data['message_id'] == 1
    
    invalid_sessionid = {'token' : encode_jwt(1,1), 'message_id' : 1}
    invalid_userid = {'token' : encode_jwt(4,0), 'message_id' : 1}

    sessionid_request = requests.delete(f"{config.url}message/remove/v1", json=invalid_sessionid)
    assert sessionid_request.status_code == 403

    authid_request = requests.delete(f"{config.url}message/remove/v1", json=invalid_userid)
    assert authid_request.status_code == 403

def test_message_remove_invalid_msgid_inchannel(clear__register3users_create4channels):
    ...
    user3_request = {'token': encode_jwt(3,0), 'message_id': 1}
    invalid_request = requests.delete(f"{config.url}message/remove/v1", json=user3_request)
    assert invalid_request.status_code == 400

def test_message_remove_not_authorised(clear__register3users_create4channels):
    """
    ...
    user1_request = {'token': encode_jwt(1,0), 'message_id': 1}
    valid_request = requests.delete(f"{config.url}message/remove/v1", json=user1_request)
    assert valid_request.status_code == 200

def test_message_remove_not_channelowner(clear__register3users_create4channels):
...
def test_remove_message(clear__register3users_create4channels):
    user1_message = {'token': encode_jwt(1,0), 'channel_id' : 1, 'message':"hey guys"}
    user2_message = {'token': encode_jwt(2,0), 'channel_id' : 1, 'message':"yooooo"}
    ...
    user1_request = {'token': encode_jwt(1,0), 'message_id': 2}
    edit_request = requests.delete(f"{config.url}message/remove/v1", json=user1_request)
